...
- 某个名称的变量搜索顺序如下`with临时切换的调用对象`->`当前闭包空间`->`父级闭包空间`->`global对象`
...
function TestVarScope(name){
    this.outer = "this.outer"
    this.msg = 'msg of this'

    this.echoAge=function(age){
        console.log(`age of ${name} is ${age}`)
    }

    this.echoMsgOfOuter = function () {
        console.log(`'outer is :${outer}`)
        console.log(`'this.outer is :${this.outer}`)
        console.log(`this.msg is :${this.msg}`)
        with (outerObj) {
            console.log(`'outer within with outerObj ':${outer}`)
        }
    }

    with(outerObj){
        this.echoMsgOfWithOuterObj = function(){
            console.log(`'outer is ':${outer}`)
            console.log(`'this.outer is ':${this.outer}`)
        }
    }
    var outer = "outer in function"
}

var test = new TestVarScope()
test.echoAge(18)
test.echoMsgOfOuter()
test.echoMsgOfWithOuterObj()
```
