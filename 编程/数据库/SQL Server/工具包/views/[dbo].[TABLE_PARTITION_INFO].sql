--	############################################################################
--	This is the view to select the table partition informaitn.
...
--  [ROWS]              : The total rows in the table partition.
--  [data]              : The total space used for the partition data in the table partition.
--  [reserved]          : The total space reserved for the partition data in the table partition.
--	############################################################################

DROP VIEW IF EXISTS [dbo].[TABLE_PARTITION_INFO];
GO

CREATE VIEW [dbo].[TABLE_PARTITION_INFO]
AS
SELECT
    ...
    ,convert(bigint,p.rows)    		as [ROWS]
    ,convert(varchar(32),used_page_count * 8) + ' KB'            as data
	,convert(varchar(32),reserved_page_count * 8) + ' KB'		as reserved
from sys.indexes i
...
left join sys.dm_db_partition_stats part_stats on
	i.object_id = part_stats.object_id 
	and p.partition_number = part_stats.partition_number
