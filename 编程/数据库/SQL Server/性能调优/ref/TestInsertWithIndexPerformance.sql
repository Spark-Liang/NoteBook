...
drop table if exists #TestTblVarInsertWithIndex;
select * into #TestTblVarInsertWithIndex from TestProperties;
...
DBCC DROPCLEANBUFFERS  --清除缓冲区
DBCC FREEPROCCACHE  --删除计划高速缓存中的元素
declare @TestTblVarWithIndexTGT as table(
	P_OPID int primary key clustered,
	O_OPID int,
	P_NAME varchar(1024),
	P_INT_VALUE int,
	P_STR_VALUE varchar(1024),
...
print 'TestTblVarWithIndex'
set statistics time on;
insert into @TestTblVarWithIndexTGT select * from #TestTblVarInsertWithIndex;
set statistics time off;
GO
...
create unique clustered index TestTmpTblSelectIntoTGT_IDX on #TestTmpTblSelectIntoTGT(P_OPID);
set statistics time off;
GO
...
create unique clustered index TestTblSelectIntoTGT_IDX on TestTblSelectIntoTGT(P_OPID);
set statistics time off;
GO